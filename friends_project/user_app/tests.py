...
class SendRequestTestCase(TestCase):
    @classmethod
    def setUpClass(cls):
        cls.tester_sender = User.objects.create(
            username="tester_sender", password="123"
        )
        cls.tester_receiver = User.objects.create(
            username="tester_receiver", password="123"
        )
    ...
    def test_send_request(self):
        response = self.client.post(
            reverse("user_app:account", kwargs={"pk": self.tester_receiver.pk}),
            data={"send_request": True},
        )
        expexted_request = FriendshipRequest.objects.get(
            request_from=self.tester_sender, request_to=self.tester_receiver
        )
...
class AutoReceivingTwoRequestsTestCase(TestCase):
    ...
    def test_auto_receiving_two_requests(self):
        ...
        response = self.client.post(
            reverse("user_app:account", kwargs={"pk": self.user_2.pk}),
            data={"send_request": True},
        )

        self.assertRedirects(
            response, reverse("user_app:account", kwargs={"pk": self.user_2.pk})
        )
        request_from_1 = FriendshipRequest.objects.get(
            request_from=self.user_1, request_to=self.user_2
        )
        self.assertIsNotNone(request_from_1)
        self.client.force_login(self.user_2)
        response = self.client.post(
            reverse("user_app:account", kwargs={"pk": self.user_1.pk}),
            data={"send_request": True},
        )
        self.assertRedirects(
            response, reverse("user_app:account", kwargs={"pk": self.user_1.pk})
        )
        request_from_2 = FriendshipRequest.objects.get(
            request_from=self.user_2, request_to=self.user_1
        )
        self.assertIsNotNone(request_from_2)
        friendship = Friendship.objects.filter(
            Q(member_1=self.user_1, member_2=self.user_2)
...
class RequestReceivingTestCase(TestCase):
    ...
    def test_request_receiving(self):
        ...
        request = FriendshipRequest.objects.get(
            request_from=self.user_1, request_to=self.user_2
        )
...
class CancelReceiversRequestTestCase(TestCase):
    ...
    def test_cancel_receivers_request(self):
        ...
        is_request = FriendshipRequest.objects.filter(
            request_from=self.user_1, request_to=self.user_2
        ).exists()
...
class RemoveFromFriendsTestCase(TestCase):
    ...
    def test_request_rejection(self):
        ...
        is_friendship = Friendship.objects.filter(
            Q(member_1=self.user_1, member_2=self.user_2)
            | Q(member_1=self.user_2, member_2=self.user_2)
        ).exists()
